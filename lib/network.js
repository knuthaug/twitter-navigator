var Network = function(spec){
    ...
    var that = spec;
    that.neo = NeoStore({"network": that});
    that.master_node = { };
    ...
    that.event_config = function(){
        return {
            ...
            onMouseEnter: function() {
                that.visual_network.canvas.getElement().style.cursor = 'move';
            },
            onMouseLeave: function() {
                that.visual_network.canvas.getElement().style.cursor = '';
            },
            //Update node positions when dragged
            onDragMove: function(node, eventInfo, e) {
                var pos = eventInfo.getPos();
                node.pos.setc(pos.x, pos.y);
                that.visual_network.plot();
            },
    ...
    that.visual_network = new $jit.ForceDirected({
                                              injectInto: 'network',
                                              width: "800",
                                              height: "600",                        
    ...
    that.handle_found_user = function(data) {
        ...
        if(data.follows){
            ...
            data.follows.forEach(function(element, index){
                                     ...
                                     that.visual_network.graph.addNode(node);
                                     that.visual_network.graph.addAdjacence(node, that.master_node);
    ...
    that.initialize_graph = function(node){
        that.visual_network.loadJSON([node]);
        node.save_master();
    };
    ...
    that.add_to_graph = function(node, to_node){
        that.visual_network.graph.addNode(node);
        node.save(to_node);
        that.visual_network.graph.addAdjacence(node, to_node);
    };

    that.compute_layout = function(){
        that.visual_network.computeIncremental({
                                            iter: 20,
                                            property: 'end',
