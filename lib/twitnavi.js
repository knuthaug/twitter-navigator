...
var NeoStore = function(spec){
    ...
    that.save_node = function(node, callback){
        return that.request("node", node, callback);
    };

    that.request = function(url_part, node, callback){

        return $.ajax( { 
                           type: "POST",
                           url: this.request_url + url_part,
                           dataType: "json",
...
var Node = function(spec){
    ...
    that.save = function(){
        var neo = NeoStore({});
        neo.save_node(that.original, that.onSave);
    };

    that.onSave = function(data){
        that.node_id = data.self;
    }
...
var Network = function(spec){
    ...
    that.handle_found_user = function(data) {
        ...
        that.initialize_graph(that.node.original);    
        twit = Twitter({});
    ...
    that.add_friends = function(data){
        nodes = data.map( function(element){
                              ...
                              //}
                              that.add_to_graph(node, that.node);
                          });
//        nodes.unshift(node);
