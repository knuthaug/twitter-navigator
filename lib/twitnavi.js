...
var NeoStore = function(spec){
    ...
    that.index_node = function(node){
        return that.post_request(this.request_url + 'index/node/twitter_id/' + node.id, this.baseurl + 'node/' + node.node_id);
    };

    that.node_exists = function(node){
        var status = that.get_request(this.request_url + 'index/node/twitter_id/' + node.id);
...
var Node = function(spec){
    ...
    that.save = function(to_node){
        var status = "";
        if(to_node && !that.neo.node_exists(that) ){
